...
impl Gear {
    ...
    // returns a cut path and a flag saying whether it was truncated at the root
    pub fn mill_offset(&self,
                       num_steps: u32,
                       mill_d: f64,
                       add_margin: f64) -> (Vec<ToothFace>, bool) {
        ...
        for i in 0..inter_face.points.len() {
            let face_x:f64 = face.points.get(i).unwrap().x;
    ...
    pub fn print_one_layer_coords(&self,
                                  ...
                                  start_z: f64,
                                  end_z: f64) -> bool {
        let mill_offset:(Vec<ToothFace>, bool) =
            self.mill_offset(num_face_steps, mill_size, mill_offset);
